	...
	const std::vector<Sphere<T> *> &spheres, const int &depth)
{
	...
	// positive.
	T bias = 1e-4f; // add some bias to the point from which we will be tracing
	bool inside = false;
	if (raydir.dot(nhit) > 0) nhit = -nhit, inside = true;
	if ((sphere->transparency > 0 || sphere->reflection > 0) && depth < MAX_RAY_DEPTH) {
		T facingratio = -raydir.dot(nhit);
		// change the mix value to tweak the effect
		T fresneleffect = (T)mix<T>(pow(1 - facingratio, 3), 1, 0.1f); 
		...
		if (sphere->transparency) {
			T ior = 1.1f, eta = (inside) ? ior : 1 / ior; // are we inside or outside the surface?
...
void render(const std::vector<Sphere<T> *> &spheres)
{
	...
	T angle = (T)tan(M_PI * 0.5f * fov / T(180));
	// Trace rays
	for (unsigned y = 0; y < height; ++y) {
		for (unsigned x = 0; x < width; ++x, ++pixel) {
			T xx = (T)((2 * ((x + 0.5) * invWidth) - 1) * angle * aspectratio);
			T yy = (T)((1 - 2 * ((y + 0.5) * invHeight)) * angle);
